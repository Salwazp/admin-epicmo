...
class EventController extends Controller
{
    ...
    {   
        ...
        $website = DB::table('websites')
            ->select(['websites.id', 'websites.url', 'websites.user_id', 'websites.exclude_bots', 'websites.exclude_ips', 'websites.exclude_params'])
            ->first();
        ...
        if ($request->input('event')) {
            ...
            if (isset($referrer['non_www_host']) == false || $referrer['non_www_host'] != $website->url) {
                ...
                // Get JSON object
                $jsondata = file_get_contents("http://ipinfo.io/" . $ip_address."/geo");
                // Decode
                $result = json_decode($jsondata, true);

                // Add the country
                $data['country'] = isset($result['city']) ? $result['city'] : 'Bandung';
